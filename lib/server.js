...
var Server = module.exports = function (options, listener){
	...
	this._socket = dgram.createSocket ("udp" + family)
			.on ("error", function (error){
				...
				//The user also can cache the requests and abort them manually
				me.emit ("error", error);
			})
			.on ("close", function (){
				me.emit ("close");
			...
			.on ("message", function (message, rinfo){
				...
				this._helpers[rinfo.address] = helper;
...
Server.prototype.getHelper = function (rinfo){
	return this._helpers[rinfo.address];
};
