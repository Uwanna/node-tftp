...
var Server = module.exports = function (options, listener, interceptor){
	...
	this._socket = dgram.createSocket ("udp" + family)
			...
			.on ("message", function (message, rinfo){
				...
				if (op === opcodes.RRQ){
					...
					var ps = new PutStream (me._currFiles, helper, message, options, gs, interceptor);
					ps.onReady = function (){
					...
					var gs = new GetStream (me._currFiles, helper, message, options, ps, interceptor);
					gs.onReady = function (){
						me.emit ("request", gs, ps);
					};
				}else{
